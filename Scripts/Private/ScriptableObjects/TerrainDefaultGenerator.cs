            ...
            for (int arrayIndex = 0; arrayIndex < VoxelPlayEnvironment.CHUNK_SIZE * VoxelPlayEnvironment.CHUNK_SIZE; arrayIndex++) {
                ...
                for (; voxelIndex > bedrockRow; voxelIndex -= ONE_Y_ROW, pos.y--) {
                    if (voxels[voxelIndex].typeIndex == Voxel.EmptyTypeIndex) { // avoid holes
                        ...
                        lastHoleIndex = voxelIndex;
                        if (pos.y <= waterLevel) { // hole under water level -> fill with water
                            voxels[voxelIndex].SetFastWater(waterVoxel);
                        }
                    }
                }
                ...
                if (lastHoleIndex >= ONE_Y_ROW && biome.vegetationDensity > 0 && voxels[lastHoleIndex - ONE_Y_ROW].opaque == VoxelPlayEnvironment.FULL_OPAQUE) {
