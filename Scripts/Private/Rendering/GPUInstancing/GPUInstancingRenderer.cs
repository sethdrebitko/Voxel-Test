...
namespace VoxelPlay.GPURendering.Instancing {
    ...
    public class GPUInstancingRenderer : IGPUInstancingRenderer {
        ...
        public GPUInstancingRenderer(VoxelPlayEnvironment env) {
            ...
            defaultInstancingMaterial[0] = Resources.Load<Material>("VoxelPlay/Materials/VP Model VertexLit");
        ...
        public void Render(Vector3 observerPos, float visibleDistance, Vector3[] frustumPlanesNormals, float[] frustumPlanesDistances) {
            ...
            for (int k = 0; k < batchedMeshes.count; k++) {
                ...
                int materialsCount = batchedMesh.materials.Length;
                for (int j = 0; j < batchedMesh.batches.count; j++) {
                    Batch batch = batchedMesh.batches.values[j];
                    if (env.instancingCullingMode != InstancingCullingMode.Disabled && GeometryUtilityNonAlloc.TestPlanesAABB(frustumPlanesNormals, frustumPlanesDistances, ref batch.boundsMin, ref batch.boundsMax, env.instancingCullingPadding)) {
                        for (int subMesh = 0; subMesh < subMeshCount; subMesh++) {
                            Material material = subMesh < materialsCount ? batchedMesh.materials[subMesh] : batchedMesh.materials[materialsCount - 1];
                            Graphics.DrawMeshInstanced(mesh, subMesh, material, batch.matrices, batch.instancesCount, batch.materialPropertyBlock, shadowCastingMode, receiveShadows, env.layerVoxels);
#if DEBUG_BATCHES
						batches++;
